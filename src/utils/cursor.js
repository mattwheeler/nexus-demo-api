...
/**
 * Generates a next_cursor from an array of activity records
 * Returns the cursor for the last record in the array
 * @param {Array} results - Array of activity records with created_at and id
 * @returns {string|null} The next cursor or null if no results
 */
function generateNextCursor(results) {
  ...
  if (!lastRecord || !lastRecord.created_at || !lastRecord.id) {
    return null;
  }
  
  return encodeCursor(lastRecord.created_at, lastRecord.id);
}

/**
 * Utility function to generate cursor from a single activity record
 * @param {Object} record - Activity record with created_at and id
 * @returns {string} The encoded cursor
 */
function cursorFromRecord(record) {
  if (!record || !record.created_at || !record.id) {
    throw new CursorError('Record must have created_at and id properties');
  }
...
/**
 * Parses cursor for database queries, returning SQL-safe values
 * @param {string} cursor - The cursor to parse
 * @returns {{created_at: string, id: string}} Parsed cursor data ready for SQL queries
 */
function parseCursorForQuery(cursor) {
